...
static void print_process_info_to_dmesg(void)
{
    ...
    pr_info("PID\tPPID\tSTATE\t\t\tPOLICY\t\t\tCPU\tMEM (KB)\tNAME\n");
    ...
    for_each_process(task)
    {
        ...
        pr_info("%d\t%d\t%-15s\t%-15s\t%llu\t%lu\t%s\n",
                task->pid,
                task->parent->pid,
                get_state_name(task->__state),
                get_policy_name(task->policy),
                utime + stime,
                (task->mm) ? task->mm->total_vm * (PAGE_SIZE / 1024) : 0,
                task->comm);
    }
...
static int proc_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "%-5s %-5s %-15s %-15s %-5s %-5s %-5s %-5s %s\n",
               "PID", "PPID", "STATE", "POLICY", "PRIO", "NICE", "CPU", "MEM (KB)", "NAME");
    ...
    for_each_process(task)
    {
        ...
        seq_printf(m, "%-5d %-5d %-15s %-15s %-5d %-5d %-5llu %-5lu %s\n",
                   task->pid,
                   task->parent->pid,
                   get_state_name(task->__state),
                   get_policy_name(task->policy),
