...
static int monitor_process(int target_pid)
{
    ...
    pr_info("  User Time:   %llu\n", utime);
    pr_info("  Kernel Time: %llu\n", stime);
    ...
    if (task->files)
    {
        ...
        for (int i = 0; i < fdt->max_fds; i++)
        {
            ...
            if (f)
            {
                ...
                if (filename)
                {
                    files_list_printed = true;
                    pr_info("  %s", f->f_path.dentry->d_name.name);
                }
            }
        }
    ...
    if (!task->files || !files_list_printed)
    {
        pr_info("  No files\n");
    }
