...
{
    lab3_kobj = kobject_create_and_add("lab03_sysfs", kernel_kobj);
    if (!lab03_kobj)
        return -ENOMEM;
...
static ssize_t do_atomic_write(struct file *file, const char __user *buffer,
                               size_t count, loff_t *pos)
{
    char input[32];
    char cmd[16];
...
static ssize_t do_atomic_read(struct file *file, char __user *buffer,
                              size_t count, loff_t *pos)
{
    char output[256];
    int len;
...
.PHONY: all clean install update-initramfs test-qemu $(SUBDIRS)
```
...
### Netlink Makefile
```makefile
# 01_communication/netlink_demo/Makefile
KERNEL_SRC=~/linux-6.16
KDIR ?= $(KERNEL_SRC)
INSTALL_PATH ?= /tmp/initramfs_lab03
...
### Synchronization Makefile
```makefile
# 02_synchronization/Makefile
KERNEL_SRC=~/linux-6.16
KDIR ?= $(KERNEL_SRC)
INSTALL_PATH ?= /tmp/initramfs_lab03
...
# Test procfs:
insmod /kernel_modules/procfs_basic.ko
echo "Hello Lab03" > /proc/lab03_basic
cat /proc/lab03_basic
rmmod procfs_basic

# Test sysfs:
insmod /kernel_modules/sysfs_demo.ko
echo 42 > /sys/kernel/lab03_sysfs/value
cat /sys/kernel/lab03_sysfs/value
rmmod sysfs_demo

# Test atomic operations:
insmod /kernel_modules/atomic_demo.ko
echo "inc" > /proc/lab03_atomic
echo "add 5" > /proc/lab03_atomic
cat /proc/lab03_atomic
rmmod atomic_demo

# Test lock comparison:
insmod kernel_modules/lock_comparison.ko
cat /proc/lab03_mutex &
cat /proc/lab03_spinlock &
rmmod lock_comparison

# Test reader-writer lock:
insmod kernel_modules/rwlock_demo.ko
cat /proc/lab03_reader &
echo 100 > /proc/lab03_writer
rmmod rwlock_demo
