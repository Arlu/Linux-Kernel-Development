...
static int __init jiffies_conversions_init(void)
{
...
static void __exit jiffies_conversions_exit(void)
{
    unsigned long total_uptime = jiffies - start_jiffies;
    
    timer_delete_sync(&time_timer);
    proc_remove(proc_entry);
    
    pr_info("TIME: Demo unloaded after %lu jiffies (%u ms)\n", total_uptime, jiffies_to_msecs(total_uptime));
}

module_init(jiffies_conversions_init);
module_exit(jiffies_conversions_exit);
