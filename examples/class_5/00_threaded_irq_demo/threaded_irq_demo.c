...
static irqreturn_t my_threaded_interrupt(int irq, void *dev_id)
{
    ...
    snprintf(shared_message, sizeof(shared_message), "Processed by thread handler #%d", local_counter);
...
{
    seq_printf(m, "echo 1 > /proc/advanced_threaded_irq_demo  # Primary only\n");
    seq_printf(m, "echo 2 > /proc/advanced_threaded_irq_demo  # Full threaded\n");
...
static ssize_t advanced_threaded_irq_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    char input[8];
    int value;
...
static int __init tirq_demo_init(void)
{
    int ret;
...
static void __exit tirq_demo_exit(void)
{
...
module_init(tirq_demo_init);
module_exit(tirq_demo_exit);
