...
static ssize_t spinlock_read(struct file *file, char __user *buffer, size_t count, loff_t *pos)
{
    char *kernel_buffer;
    int len, local_data;
    ...
    len = snprintf(kernel_buffer, 256, "Shared data: %d\nOperations: %lu\n", local_data, local_count);
...
static ssize_t spinlock_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    char input[32];
    int value;
