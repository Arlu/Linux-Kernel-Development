...
// Mutex demonstration:
static ssize_t mutex_demo_read(struct file *file, char __user *buffer, size_t count, loff_t *pos)
{
    char output[256], local_msg[64];
    int len, local_data;
    ...
    snprintf(shared_message, sizeof(shared_message), "Updated by PID %d", current->pid);
...
static ssize_t semaphore_demo_read(struct file *file, char __user *buffer, size_t count, loff_t *pos)
{
    char output[256];
...
// Status overview:
static ssize_t status_read(struct file *file, char __user *buffer, size_t count, loff_t *pos)
{
    char output[256];
    int len;
