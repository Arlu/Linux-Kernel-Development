...
static void add_log_entry(const char *message)
{
   if (log_count < MAX_LOG_ENTRIES)
   {
       ...
       strncpy(log_buffer[log_count].message, message, sizeof(log_buffer[log_count].message) - 1);
   ...
   else
   {
       ...
       memmove(&log_buffer[0], &log_buffer[1], sizeof(struct log_entry) * (MAX_LOG_ENTRIES - 1));
       log_buffer[MAX_LOG_ENTRIES - 1].timestamp = jiffies;
       log_buffer[MAX_LOG_ENTRIES - 1].entry_id = next_id++;
       strncpy(log_buffer[MAX_LOG_ENTRIES - 1].message, message, sizeof(log_buffer[MAX_LOG_ENTRIES - 1].message) - 1);
...
static ssize_t log_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
   char *kernel_buffer;
